...
void uiLcdDrawSymbol(byte col,byte row,byte sid)
{
	...
	if(sid<sizeof(SymbolMaps)/sizeof(char*)){
		...
		display.setColor(WHITE);
		display.drawXbm(x,y,FontWidth,FontHeight,(const uint8_t*)SymbolMaps[_reservedChar2SymbolId[sid]]);
	}else{
	}
	display.display();
}
void uiLcdDrawSymbolBmp(byte col,byte row,SymbolId symbol)
{
	...
	display.setColor(WHITE);
	display.drawXbm(XofCol(col),YofRow(row),FontWidth,FontHeight,(const uint8_t*)SymbolMaps[symbol]);
	display.display();
}
